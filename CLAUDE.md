...
- **references/localization.md** — The full opt-in localization phase (offered after a device's English set is approved). Translates headlines and regenerates one screenshot per locale, reusing the approved English screenshot as the style template. Outputs under `final/<locale>/`.
- **compose_common.py** — Shared Pillow-only module (PEP 723 header) holding the `ComposeConfig` frozen dataclass, the `compose()` compositing routine, and the `word_wrap`/`fit_font`/`draw_centered`/`parse_hex_color`/`die`/`warn` helpers that both compose scripts import. The device frame slides down dynamically: `compose()` measures the headline block from `text_top`, then sets `device_y = max(cfg.device_y, text_bottom + min_text_device_gap)` so a tall/wrapping headline never overlaps the frame. Also validates `--bg` (hex, with 3-digit shorthand) and `--screenshot` existence.
